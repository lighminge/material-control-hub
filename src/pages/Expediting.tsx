          ...
          {[1, 2, 3, 4, 5, 6, 7].map(d => {
            ...
            return (
              <div key={d} className="flex items-center gap-2">
                <div className={`w-4 h-4 rounded-full border ${getColorClass(d)}`}></div>
                <span className="font-medium text-sm">{label} ({count})</span>
              </div>
            );
          })}
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center h-24">目前尚無未補完之管制單</TableCell>
                </TableRow>
              ) : (
                controls.sort((a, b) => calculateDays(b.startDate) - calculateDays(a.startDate)).map((control, index) => {
                  const days = calculateDays(control.startDate);
                  const colorClass = getColorClass(days >= 7 ? 7 : days);
                  return (
                    <TableRow key={control.id}>
                      ...
                      <TableCell className="text-green-600 font-bold">{control.items.filter((i: any) => i.missingQuantity === 0).length}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Badge className={`text-sm px-3 py-1 ${colorClass}`} variant="outline">
                          {days} 天
                        </Badge>
                      </TableCell>
                    </TableRow>
